...
extension Theme.Collection {
    ...
    public func themeStylesIfLoaded(for object: NSObject) -> Theme.Style.Collection? {
        ...
        for item in self.themeStyles {
            guard item.identifier.contains(identifier) else { continue }
            if themeStyles != nil {
                themeStyles!.union(item)
            } else {
                themeStyles = Theme.Style.Collection.init(for: object, union: item)
            }
        }
